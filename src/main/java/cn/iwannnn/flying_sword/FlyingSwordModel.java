...
public class FlyingSwordModel extends EntityModel<FlyingSwordEntity> {
    ...
    public FlyingSwordModel() {
        textureWidth = 128;
        textureHeight = 128;// 设置材质大小

        body = new ModelRenderer(this);
        body.setRotationPoint(13.0F, 24.0F, -13.0F);// 设置旋转点
        body.setTextureOffset(0, 50);// 设置uv位置
        body.addBox(-1.1053F, -16.0F, -2.7368F, 4, 2, 6, 0.0F, false);// 前三个参数是空间中的位置，后三个参数是方块大小，最后两个一个是控制透明度，另一个是控制镜像的
    ...
    @Override
    public void setRotationAngles(FlyingSwordEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks,
